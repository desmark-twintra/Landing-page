...
/**
 * Vendored from React Bits (DepthCarousel, JS + CSS variant) and adapted for
 * this repo: typed props, `cn()` for className merging, and the upstream
 * wheel-to-navigate handler removed — it calls `preventDefault()` on every
 * wheel event unconditionally, which would block the page's real scroll
 * container (`AppScrollArea`, see AGENTS.md) whenever the cursor sits over
 * the hero. Drag, arrows, dots, keyboard, and autoplay already cover
...
export function DepthCarousel({
  ...
  // Kept in sync every render (not just on prop changes) so the stable,
  // empty-deps callbacks below always read the latest values without
  // needing to be recreated — a ref update belongs in an effect, not render.
  useEffect(() => {
    ...
    >
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            {/* eslint-disable-next-line @next/next/no-img-element -- geometry is driven imperatively by layout(), not compatible with next/image's fill model */}
